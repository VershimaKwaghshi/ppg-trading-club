...
function useInView(ref, threshold) {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => { if (entries[0].isIntersecting) setInView(true); }, { threshold: t });
...
function AnimatedSection({ children, className = "", delay = 0 }) {
  ...
  return (
    <div ref={ref} className={className} style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(20px)" : "translateY(0)", transition: `opacity 0.5s ease ${delay}s, transform 0.5s ease ${delay}s` }}>
      {children}
    </div>
  );
}
...
function ResponsiveBrandLogo() {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "2px", userSelect: "none" }}>
      <div className="shimmer-text" style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "24px", fontWeight: "700", fontStyle: "italic", letterSpacing: "1px" }}>
        Trading Club
      </div>
      <div style={{ display: "flex", alignItems: "center", width: "100%", gap: "6px" }}>
        ...
        <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7.5px", fontWeight: "700", color: "#e8d080", letterSpacing: "2.5px", whiteSpace: "nowrap", opacity: 0.95 }}>
...
function Navbar({ onOpenRegister }) {
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, background: "#050814", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(196,160,80,0.12)", padding: "0 1.25rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 75 }}>
        ...
        <div className={`nav-links ${menuOpen ? "open" : ""}`} style={{ display: "flex", gap: "32px", alignItems: "center" }}>
          {NAV_LINKS.map((l) => <a key={l.label} href={l.href} style={{ color: "#b0a080", fontSize: 14, fontFamily: "'DM Sans', sans-serif", textDecoration: "none", fontWeight: 500 }}>{l.label}</a>)}
          <button onClick={onOpenRegister} style={{ background: "linear-gradient(135deg,#c4a050,#f0d080)", color: "#050814", fontWeight: 700, fontSize: 13, padding: "11px 22px", borderRadius: 6, border: "none", cursor: "pointer" }}>Join Now</button>
        </div>
      </div>
      <style>{`
        .shimmer-text {
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050814", color: "#f0e8d0", overflowX: "hidden", paddingTop: "75px" }}>
